...
//Função auxiliar que identifica linhas horizontais
int lookaround(int linha, int coluna, viewport * port, bufferdevice * dev, int colorBorder, int pixels){
	int i, cont;
	cont = 0;
	
	i = coluna;
	while(i < coluna + pixels){
		//Verifica se há pixels continuos. Caso tenha algum buraco, zera contador
		if(dev->buffer[linha*dev->MaxX + i] == colorBorder){cont++;}else{cont = 0;}
		i++;
	}
...
//Função auxiliar que identifica vertice
int vertice (int linha, int coluna, viewport * port, bufferdevice * dev, int colorBorder){
	int i,h,v, f1,f2,f3;
	f1 = 0;
	f2 = 0;
	f3 = 0;
	...
	for(i = coluna; i < coluna + 5; i++){
		...
		if(dev->buffer[linha*dev->MaxX + i] == colorBorder){f1 = 1;}
		if((dev->buffer[linha*dev->MaxX + i] != colorBorder)&&(f1)){f2 = 1;}
		if((dev->buffer[linha*dev->MaxX + i] == colorBorder)&&(f2)){f3 = 1;}
	}
...
int Preenchimento(viewport * port, bufferdevice * dev, int colorBorder, int color){
    ...
    border = 0;
	    for(i=port->xmin + 1;i<port->xmax;i++) {
	    	...
	    	if(dev->buffer[j * dev->MaxX + i] == colorBorder){
	    		...
	    		if(lookaround(j, i, port, dev, colorBorder,3)){
	    			//É uma linha horizontal
	    			h = 1;
	    		}
	    		if((!border)&&(h!=1)){
	    			//Verifica se é um vertice
	    			v = vertice(j, i, port, dev, colorBorder);
	 				//Só muda paridade se não for vertice
	    			if(v != 1){paridade = !paridade;}
	    			//É uma borda
	    			border = 1;
	    		}
	    	}else{
